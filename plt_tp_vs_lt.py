...
def plot_line_segments(csv_file1, csv_file2, fig_file_name):
...
def plot_lines(csv_file, ax, left_or_right):
    ...
    if (left_or_right == 0):
        legend = ax.legend(bbox_to_anchor=(1.05, 1.25), ncol=9, loc='upper center') 
        ax.set_ylabel("Latency (ms)")
        ax.set_title("PN Counter", y=-0.01)
    else:
        ax.set_title("OR Set", y=-0.01)

    if (left_or_right == 1):
        ax.set_xticks([40000, 80000, 120000, 160000, 200000])

def plt_to_file(fig, fig_file_name):
    fig.savefig(fig_file_name, dpi=300, format="pdf", bbox_inches="tight")

if __name__ == "__main__":
    plot_line_segments("results/pnc_tp_vs_lt.csv", "results/orset_tp_vs_lt.csv", "results/tp_vs_lt.pdf")
